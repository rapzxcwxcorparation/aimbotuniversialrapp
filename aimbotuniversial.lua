if getgenv().RapzX_Loaded then
    kirimNotif("⚠️ SCRIPT ALREADY LOADED", "Script sudah berjalan! Unload dulu atau reload game.")
    return
end
getgenv().RapzX_Loaded = true
...
local R6_AIM_PART_NAMES = {
    "Head (R6)", "Torso (R6)", "L Leg (R6)", "R Leg (R6)"
}
...
local R15_AIM_PART_NAMES = {
    "Head (R15)", "UprTorso (R15)", "LwrTorso (R15)",
    "L UprLeg (R15)", "R UprLeg (R15)",
...
local CHAMS_OUTLINE_COLOR_NAMES = { "Merah", "Hijau", "Biru", "Kuning", "Ungu", "Putih" }

local Outline3D = { Color = Color3.fromRGB(255, 0, 0), Thickness = 2, Transparency = 0.3, WhitelistColor = Color3.fromRGB(0, 255, 0) }

local Config = {
    Aimbot = {
        Enabled = true, TeamCheck = false, WallCheck = true,
        ...
        HumanizeAimEnabled = false,
        HumanizeIntensity = 3,
        HumanizeCurveChance = 0.4,
        HumanizeMicroCorrection = 0.3
    },
    ESP = {
        Enabled = true, BoxEnabled = false, SkeletonEnabled = false, TracerEnabled = false,
        ...
        ChamsEnabled = false, ChamsFillEnabled = false, ChamsFillTransparency = 0.5,
        ChamsOutlineEnabled = false, ChamsOutlineTransparency = 0,
        ChamsOutlineColor = Color3.fromRGB(255, 255, 255), ChamsOutlineThickness = 0.5,
        NoTeamColorPreset = 1,
        ESPPersistence = true,
        ESPTimeout = 0.5,
        ForceLoadRange = 500,
    ...
    Misc = {
        ...
        AutoGoldenKnife = false, DimLighting = true, InfiniteAmmo = false,
        AntiLag = false
    }
}

local STATE = {
    ESPPool = {}, ESPActive = {}, ESPPersistence = {}, ChamsActive = {}, ESPStudsObjects = {},
...
local function getDynamicAimPart(player)
    ...
    local priorityParts = isR15 and 
        {"Head", "UpperTorso", "LowerTorso", "LeftUpperLeg", "RightUpperLeg", "LeftLowerLeg", "RightLowerLeg"} or
        {"Head", "Torso", "Left Leg", "Right Leg"}
...
local function getClosestInFOV(ignorePlayer)
    ...
    if STATE.CurrentTarget and isPlayerAlive(STATE.CurrentTarget) and STATE.CurrentTarget ~= ignorePlayer then
        ...
        local part = STATE.CurrentTarget.Character and STATE.CurrentTarget.Character:FindFirstChild(aimPartName)
...
local function getEffectiveColor(player)
    if player.Team and STATE.IsTeamGame then
...
local function updateBigHead()
    if not Config.Aimbot.BigHeadEnabled or not Config.Aimbot.Enabled then return end
...
local function updateOutline3D()
    if not Config.ESP.Enabled then
        for _, h in pairs(STATE.OutlineActive) do
            if h then
                pcall(function()
                    h:Destroy()
                end)
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if not char or not hrp or not isPlayerAlive(p) then
            if STATE.OutlineActive[p] then
                STATE.OutlineActive[p].Enabled = false
            end
            continue
        end
        ...
        local teamOk = true
        if STATE.IsTeamGame and Config.ESP.AutoTeamDetect and p.Team == LocalPlayer.Team and not Config.ESP.ForceShowAll then
            teamOk = false
        end

        local isWhite = isWhitelisted(p)

        if dist <= Config.ESP.MaxDistance and teamOk then
            local color = isWhite and Outline3D.WhitelistColor or Outline3D.Color

            if not STATE.OutlineActive[p] or not STATE.OutlineActive[p].Parent then
                if STATE.OutlineActive[p] then
                    pcall(function()
                        STATE.OutlineActive[p]:Destroy()
                    end)
                end
                ...
                pcall(function()
                    h.OutlineThickness = Outline3D.Thickness
                end)
                STATE.OutlineActive[p] = h
            end

            local h = STATE.OutlineActive[p]
            if h and h.Parent then
                h.Enabled = true
                h.OutlineColor = color
                h.OutlineTransparency = Outline3D.Transparency
                pcall(function()
                    h.OutlineThickness = Outline3D.Thickness
                end)
                h.FillTransparency = 1
    ...
    for p, h in pairs(STATE.OutlineActive) do
        if not currentPlayers[p] or not p.Parent or not isPlayerAlive(p) then
            pcall(function()
                h:Destroy()
            end)
            STATE.OutlineActive[p] = nil
        end
    end
end

local function waitForCharacter(player, maxWait)
...
Players.PlayerAdded:Connect(function(player)
    ...
    task.spawn(function()
        local character = waitForCharacter(player, 15)
...
local function updateESP()
    ...
    for player, data in pairs(STATE.ESPPersistence) do
        if now - data.lastSeen < Config.ESP.ESPTimeout then
            data.stillValid = true
        else
            data.stillValid = false
        end
    end
    
    for _, e in ipairs(STATE.ESPPool) do 
        if e then e.assigned = false end 
    end
    ...
    for _, p in ipairs(STATE.PlayerCache) do
        ...
        if hrp and (myHRP.Position - hrp.Position).Magnitude <= Config.ESP.MaxDistance then
            ...
            if onScreen then
                table.insert(visibleData, { player = p, lx = lx, rx = rx, cy = minY, by = maxY, hrpPos = hrp.Position })
                STATE.ESPPersistence[p] = { lastSeen = now, stillValid = true, lastBox = { lx = lx, rx = rx, cy = minY, by = maxY }, lastHRPPos = hrp.Position }
            elseif Config.ESP.ESPPersistence and STATE.ESPPersistence[p] and STATE.ESPPersistence[p].stillValid then
                local pd = STATE.ESPPersistence[p]
                table.insert(visibleData, { player = p, lx = pd.lastBox.lx, rx = pd.lastBox.rx, cy = pd.lastBox.cy, by = pd.lastBox.by, hrpPos = pd.lastHRPPos, persisted = true })
            end
        end
    end
    
    for p, data in pairs(STATE.ESPPersistence) do
        if now - data.lastSeen > Config.ESP.ESPTimeout + 1.0 then
            STATE.ESPPersistence[p] = nil
        end
    end
    ...
    for i, data in ipairs(visibleData) do
        ...
        if data.persisted then
            local alpha = 1 - math.min((now - STATE.ESPPersistence[p].lastSeen) / Config.ESP.ESPTimeout, 1)
            for _, l in ipairs(entity.Box) do l.Transparency = 1 - (Config.ESP.Transparency * alpha) end
            entity.Tracer.Transparency = 1 - (Config.ESP.Transparency * alpha)
        end
        ...
        if Config.ESP.BoxEnabled then
            ...
            for _, l in ipairs(entity.Box) do l.Visible = true; l.Color = bc; if not data.persisted then l.Transparency = 1 - Config.ESP.Transparency end end
        else for _, l in ipairs(entity.Box) do l.Visible = false end end
        if Config.ESP.SkeletonEnabled and not data.persisted then
        ...
        if Config.ESP.TracerEnabled then entity.Tracer.From = Cam.ViewportSize / 2; entity.Tracer.To = Vector2.new(cx, by); entity.Tracer.Visible = true; entity.Tracer.Color = bc; if not data.persisted then entity.Tracer.Transparency = 1 - Config.ESP.Transparency end else entity.Tracer.Visible = false end
        ...
        if hum and Config.ESP.HealthBarEnabled and not data.persisted then
...
local function CreateSensorBox()
    if STATE.SensorBox then STATE.SensorBox:Destroy() end
    local myChar = LocalPlayer.Character
    if not myChar then return end
    local myHRP = myChar:FindFirstChild("HumanoidRootPart")
    if not myHRP then return end
    
    STATE.SensorBox = Instance.new("Part")
    STATE.SensorBox.Name = "SensorBox"
    STATE.SensorBox.Size = Vector3.new(30, 30, 30)
    STATE.SensorBox.CFrame = myHRP.CFrame
    STATE.SensorBox.Anchored = true
    STATE.SensorBox.CanCollide = false
    STATE.SensorBox.Transparency = 0.7
    STATE.SensorBox.Color = Color3.fromRGB(0, 255, 0)
    STATE.SensorBox.Material = Enum.Material.ForceField
    STATE.SensorBox.Parent = workspace
    
    local box = Instance.new("SelectionBox", STATE.SensorBox)
    box.Adornee = STATE.SensorBox
    box.Color3 = Color3.fromRGB(0, 255, 0)
    box.LineThickness = 0.1
    box.Transparency = 0.5
end

local function RemoveSensorBox()
    if STATE.SensorBox then
        STATE.SensorBox:Destroy()
        STATE.SensorBox = nil
    end
...
local function handleRespawn()
    if not Config.AutoPlay.AutoRespawn then return end
...
local function autoPlayLoop()
    while Config.AutoPlay.Enabled and STATE.AutoPlayThread do
        local char = LocalPlayer.Character; local humanoid = char and char:FindFirstChildOfClass("Humanoid"); local hrp = char and char:FindFirstChild("HumanoidRootPart")
        ...
        if STATE.currentAutoState == "Combat" then
            ...
            if Config.AutoPlay.HumanMode and Config.AutoPlay.JiggleAim and STATE.AutoTarget then
                local aimPartName = getAimPart(STATE.AutoTarget)
                local part = STATE.AutoTarget.Character and STATE.AutoTarget.Character:FindFirstChild(aimPartName)
...
local function startAutoPlay() 
    if not STATE.AutoPlayThread then 
        STATE.AutoPlayThread = task.spawn(autoPlayLoop) 
    end 
end
...
local function stopAutoPlay()
    ...
    STATE.AutoTarget = nil
    STATE.currentAutoState = "Roaming"
    STATE.roamingTarget = nil
    pcall(function() 
...
local function stopAutoGoldenKnife() STATE.AutoGoldenKnifeThread = nil end
...
local function updateCrosshair()
    if not Config.Crosshair.Enabled then
        ...
        if STATE.CrosshairCircle then STATE.CrosshairCircle.Visible = false end
        return
    end
...
STATE.InputBeganConnection = UserInputService.InputBegan:Connect(function(inp, proc)
    ...
    if isTriggerKeyPressed(inp) then 
        STATE.LeftClickHeld = true
        local tempTarget, _ = getClosestInFOV()
        STATE.LockedTarget = tempTarget
    end
end)
...
local function AddDropdown(page, text, options, defaultIndex, callback)
    local holder = Instance.new("Frame", page)
...
local function setAntiLag(enabled)
    if enabled then
        STATE.antiLagBackup = {
            ...
            ChamsEnabled = Config.ESP.ChamsEnabled,
            CyberVision = Config.CyberVision.Enabled,
            ShowFOV = Config.Aimbot.ShowFOV,
            Spinbot = Config.Aimbot.SpinbotEnabled,
            RGBWeapon = Config.Misc.RGBWeapon,
            MaxDistance = Config.ESP.MaxDistance,
            MaxESP = STATE.MaxESPEntities
        }
        ...
        if STATE.antiLagBackup.ESPEnabled ~= nil then
            Config.ESP.Enabled = STATE.antiLagBackup.ESPEnabled
            Config.ESP.ChamsEnabled = STATE.antiLagBackup.ChamsEnabled
            Config.CyberVision.Enabled = STATE.antiLagBackup.CyberVision
            Config.Aimbot.ShowFOV = STATE.antiLagBackup.ShowFOV
            Config.Aimbot.SpinbotEnabled = STATE.antiLagBackup.Spinbot
            Config.Misc.RGBWeapon = STATE.antiLagBackup.RGBWeapon
            Config.ESP.MaxDistance = STATE.antiLagBackup.MaxDistance
            STATE.MaxESPEntities = STATE.antiLagBackup.MaxESP
        end
    end
end
...
local AimModeDropdown = AddDropdown(AimPage, "Aim Mode", AIM_MODES, 1, function(idx, val)
    Config.Aimbot.AimMode = val
    updateTargetPartHUD()
    if val == "Dynamic" then
        kirimNotif("AIM MODE", "Dynamic - otomatis pilih part terdekat crosshair (Head/Torso/Legs)")
    else
...
local AutoFireModeDropdown = AddDropdown(AimPage, "Auto Fire", AUTO_FIRE_MODES, 1, function(idx, val)
    Config.Aimbot.AutoFireMode = val
end)
...
AddButton(AimPage, "WHITELIST ALL NEARBY", function()
    local myChar = LocalPlayer.Character
    if not myChar then kirimNotif("ERROR", "No character!"); return end
    local myHRP = myChar:FindFirstChild("HumanoidRootPart")
    if not myHRP then kirimNotif("ERROR", "No HRP!"); return end
    ...
    for _, player in pairs(Players:GetPlayers()) do
        ...
        if dist <= 15 then
            ...
            table.insert(detected, name .. " (" .. math.floor(dist) .. "s)")
            
            if not Config.Aimbot.TeamCheckWhitelistEnabled then
                Config.Aimbot.TeamCheckWhitelistEnabled = true
            end
            if not table.find(Config.Aimbot.TeamCheckWhitelist, name) then
                table.insert(Config.Aimbot.TeamCheckWhitelist, name)
                count = count + 1
            end
        end
    end
    ...
    if count == 0 then
        kirimNotif("BOMB", "No players in sensor area!\nSensor box visible for 3 seconds.")
    else
        kirimNotif("BOMB", count .. " players whitelisted!\n" .. table.concat(detected, ", "))
    end
...
AddButton(AimPage, "SHOW SENSOR AREA", function()
    if STATE.SensorEnabled then
        RemoveSensorBox()
        kirimNotif("SENSOR", "Sensor OFF")
    else
        CreateSensorBox()
        STATE.SensorEnabled = true
        kirimNotif("SENSOR", "Sensor ON - Area 30 studs visible!")
    end
end)

AddButton(AimPage, "HIDE SENSOR AREA", RemoveSensorBox)

AddButton(AimPage, "CLEAR WHITELIST", function()
    local count = #Config.Aimbot.TeamCheckWhitelist
    Config.Aimbot.TeamCheckWhitelist = {}
    Config.Aimbot.TeamCheckWhitelistEnabled = false
    kirimNotif("CLEAR", count .. " players removed!")
end)

AddButton(AimPage, "LIST WHITELIST", function()
    local names = table.concat(Config.Aimbot.TeamCheckWhitelist, ", ")
    if names == "" then
        kirimNotif("LIST", "No players in whitelist!")
    else
        kirimNotif("LIST ("..#Config.Aimbot.TeamCheckWhitelist..")", names)
    end
end)
...
AddToggle(ESPPage, "ESP Persistence", Config.ESP.ESPPersistence, function(v) Config.ESP.ESPPersistence = v end, { "ESP", "ESPPersistence" })
AddSlider(ESPPage, "Persistence Timeout", 0.1, 2.0, Config.ESP.ESPTimeout, 1, function(v) Config.ESP.ESPTimeout = v end, { "ESP", "ESPTimeout" })
AddSlider(ESPPage, "Force Load Range", 100, 1000, Config.ESP.ForceLoadRange, 0, function(v) Config.ESP.ForceLoadRange = v end, { "ESP", "ForceLoadRange" })
AddToggle(ESPPage, "ESP Chams", Config.ESP.ChamsEnabled, function(v) Config.ESP.ChamsEnabled = v end, { "ESP", "ChamsEnabled" })
AddToggle(ESPPage, "Chams Fill", Config.ESP.ChamsFillEnabled, function(v) Config.ESP.ChamsFillEnabled = v end, { "ESP", "ChamsFillEnabled" })
AddSlider(ESPPage, "Chams Fill Alpha", 0, 100, math.floor(Config.ESP.ChamsFillTransparency * 100), 0, function(v) Config.ESP.ChamsFillTransparency = v / 100 end, { "ESP", "ChamsFillTransparency" })
AddToggle(ESPPage, "Chams Outline", Config.ESP.ChamsOutlineEnabled, function(v) Config.ESP.ChamsOutlineEnabled = v end, { "ESP", "ChamsOutlineEnabled" })
AddSlider(ESPPage, "Outline Thickness", 0.1, 2.0, Config.ESP.ChamsOutlineThickness, 1, function(v) Config.ESP.ChamsOutlineThickness = v end, { "ESP", "ChamsOutlineThickness" })
local ChamsOutlineColorDropdown = AddDropdown(ESPPage, "Outline Color", CHAMS_OUTLINE_COLOR_NAMES, 6, function(idx, val) Config.ESP.ChamsOutlineColor = CHAMS_OUTLINE_COLORS[idx] end)
AddSlider(ESPPage, "Outline 3D Thickness", 0.5, 5, Outline3D.Thickness, 1, function(v) Outline3D.Thickness = v; for _, h in pairs(STATE.OutlineActive) do if h then h.OutlineThickness = v end end end)
AddSlider(ESPPage, "Outline 3D Transparency", 0, 100, math.floor(Outline3D.Transparency * 100), 0, function(v) Outline3D.Transparency = v / 100; for _, h in pairs(STATE.OutlineActive) do if h then h.OutlineTransparency = v / 100 end end end)
local OutlineColorDropdown = AddDropdown(ESPPage, "Outline 3D Color", CHAMS_OUTLINE_COLOR_NAMES, 6, function(idx, val) Outline3D.Color = CHAMS_OUTLINE_COLORS[idx]; for _, h in pairs(STATE.OutlineActive) do if h then local p = Players:GetPlayerFromCharacter(h.Adornee); if p and isWhitelisted(p) then h.OutlineColor = Outline3D.WhitelistColor else h.OutlineColor = Outline3D.Color end end end end)
local WhitelistOutlineColorDropdown = AddDropdown(ESPPage, "Whitelist Outline Color", CHAMS_OUTLINE_COLOR_NAMES, 2, function(idx, val) Outline3D.WhitelistColor = CHAMS_OUTLINE_COLORS[idx]; for _, h in pairs(STATE.OutlineActive) do if h then local p = Players:GetPlayerFromCharacter(h.Adornee); if p and isWhitelisted(p) then h.OutlineColor = Outline3D.WhitelistColor end end end end)
...
local NoTeamColorDropdown = AddDropdown(ESPPage, "No Team Color", NO_TEAM_COLOR_NAMES, Config.ESP.NoTeamColorPreset, function(idx, val) Config.ESP.NoTeamColorPreset = idx end)
...
local CrosshairStyleDropdown = AddDropdown(CrosshairPage, "Style", CROSSHAIR_STYLES, 1, function(idx, val)
    Config.Crosshair.Style = val
end)
...
local CrosshairColorDropdown = AddDropdown(CrosshairPage, "Color", CROSSHAIR_COLOR_NAMES, Config.Crosshair.ColorPreset, function(idx, val)
    Config.Crosshair.ColorPreset = idx
end)
...
AddDropdown(MiscPage, "Teleport Hit", {"Left", "Right"}, Config.Misc.TeleportHitButton == "Left" and 1 or 2, function(idx, val)
    Config.Misc.TeleportHitButton = val
end)
...
local PresetDropdown = AddDropdown(MiscPage, "Preset", {"RAGE", "LEGIT", "SAFE", "CUSTOM"}, 1, function(idx, val)
    STATE.currentPresetSlot = val
end)
    ...
    if STATE.originalRootJointC0 then
        pcall(function()
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                local rootJoint = LocalPlayer.Character.HumanoidRootPart:FindFirstChild("RootJoint")
                if rootJoint then rootJoint.C0 = STATE.originalRootJointC0 end
            end
        end)
    end
    ...
    while task.wait(0.5) do
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and isPlayerAlive(p) then
                ...
                if p.Team then teams[p.Team.Name or tostring(p.Team)] = true end
                
                if not STATE.ESPStudsObjects[p] and Config.ESP.DistanceEnabled and Config.ESP.Enabled then
                    createDistanceDisplay(p)
                end
                
                if Config.ESP.Enabled and not STATE.forceLoadChecked[p] then
                    local hrp = p.Character and p.Character:FindFirstChild("HumanoidRootPart")
                    local myHRP = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                    if hrp and myHRP then
                        local dist = (myHRP.Position - hrp.Position).Magnitude
                        if dist <= Config.ESP.ForceLoadRange then
                            STATE.forceLoadChecked[p] = true
                            STATE.ESPPersistence[p] = {
                                lastSeen = tick(),
                                stillValid = true,
                                lastBox = { lx = 0, rx = 0, cy = 0, by = 0 },
        ...
        for p, _ in pairs(STATE.forceLoadChecked) do
            if not p.Parent or not isPlayerAlive(p) then
                STATE.forceLoadChecked[p] = nil
            end
        end
        ...
        if Config.ESP.Enabled and Config.ESP.ChamsEnabled then
            ...
            for _, p in ipairs(STATE.PlayerCache) do
                activePlayerSet[p] = true
                local teamOk = true
                if STATE.IsTeamGame and Config.ESP.AutoTeamDetect and p.Team == LocalPlayer.Team and not Config.ESP.ForceShowAll then teamOk = false end
                local hrp = p.Character and p.Character:FindFirstChild("HumanoidRootPart")
                local myHRP = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                local distOk = hrp and myHRP and (myHRP.Position - hrp.Position).Magnitude <= Config.ESP.MaxDistance
                
                if teamOk and distOk then
                    if not STATE.ChamsActive[p] and p.Character then
                        local h = Instance.new("Highlight")
                        h.Adornee = p.Character
                        h.Parent = CoreGui
                        h.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
                        STATE.ChamsActive[p] = h
                    end
                    local h = STATE.ChamsActive[p]
                    if h then
                        h.Enabled = true
                        local bc = getEffectiveColor(p)
                        
                        if Config.ESP.ChamsFillEnabled then
                            h.FillColor = bc
                            h.FillTransparency = Config.ESP.ChamsFillTransparency
                        else
                            h.FillTransparency = 1
                        end
                        
                        if Config.ESP.ChamsOutlineEnabled then
                            h.OutlineColor = Config.ESP.ChamsOutlineColor
                            h.OutlineTransparency = Config.ESP.ChamsOutlineTransparency
                        else
                            h.OutlineTransparency = 1
                        end
                    end
                else
                    if STATE.ChamsActive[p] then STATE.ChamsActive[p].Enabled = false end
                end
            end
            for p, h in pairs(STATE.ChamsActive) do
                if not activePlayerSet[p] then
                    h:Destroy()
                    STATE.ChamsActive[p] = nil
                end
            end
        else
            for p, h in pairs(STATE.ChamsActive) do
...
Players.PlayerRemoving:Connect(function(p)
    if STATE.OutlineActive[p] then
        pcall(function() STATE.OutlineActive[p]:Destroy() end)
        STATE.OutlineActive[p] = nil
    end
    if STATE.ChamsActive[p] then 
        STATE.ChamsActive[p]:Destroy()
        STATE.ChamsActive[p] = nil 
    end
    if STATE.ESPActive[p] then STATE.ESPActive[p] = nil end
    if STATE.ESPStudsObjects[p] then 
        if STATE.ESPStudsObjects[p].GUI then STATE.ESPStudsObjects[p].GUI:Destroy() end
        STATE.ESPStudsObjects[p] = nil 
    end
    STATE.ESPPersistence[p] = nil
    STATE.forceLoadChecked[p] = nil
end)

RunService:BindToRenderStep("RapzX_FOV_Force", Enum.RenderPriority.Camera.Value + 1, function()
    Cam = workspace.CurrentCamera
    if Config.Misc.FOVChangerEnabled then Cam.FieldOfView = Config.Misc.CameraFOV end
end)

STATE.RenderConnection = RunService.RenderStepped:Connect(function()
    if tick() - STATE.lastESPUpdate > 0.033 then
        updateESP()
    ...
    if flickTarget and isPlayerAlive(flickTarget) then
        ...
        elseif Config.Aimbot.Enabled and frontTarget and isPlayerAlive(frontTarget) then
            local aimPartName = getAimPart(frontTarget)
            local part = frontTarget.Character and frontTarget.Character:FindFirstChild(aimPartName)
            if part and part.Position.Y > -50 then
                if Config.Aimbot.TriggerMode then
                    if STATE.LeftClickHeld and STATE.LockedTarget and isPlayerAlive(STATE.LockedTarget) then
                        local lockAimPart = getAimPart(STATE.LockedTarget)
                        local lockPart = STATE.LockedTarget.Character and STATE.LockedTarget.Character:FindFirstChild(lockAimPart)
        ...
        if Config.Aimbot.Enabled then
            ...
            elseif autoMode == "Scope & Shoot" then
                if canFire then
                    if not STATE.isScoping and (tick() - STATE.scopeCooldown) >= Config.Aimbot.ScopeCooldown then
...
STATE.HeartbeatConnection = RunService.Heartbeat:Connect(function()
    applyRGBWeapon(); updateAutoBhop(); updateAutoStrafe(); updateAntiAim(); updateCyberVision(); updateSpinbot(); applyWalkSpeedAndJump()
    
    if tick() - STATE.lastOutlineUpdate > 0.1 then
        updateOutline3D()
        STATE.lastOutlineUpdate = tick()
    end
    
    if STATE.SensorBox and STATE.SensorBox.Parent then
        local myHRP = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if myHRP then STATE.SensorBox.CFrame = myHRP.CFrame end
    end
end)
